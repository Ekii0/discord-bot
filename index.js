      ...
      .then((json) => {
        ...
        fetch(metadata_query_url, {
          ...
          .then((json) => {
            ...
            const uri = json[0]?.onChainMetadata?.metadata?.data?.uri;
            console.log(`Name: ${name} | Symbol: ${symbol}`);

            // fetch(uri)
            //   .then((res) => res.json())
            //   .then((json) => {
            let description = json[0]?.offChainMetadata?.metadata?.description;
            if (description == undefined || description == "") {
              description = "N/A";
            }
            let img_uri = json[0]?.offChainMetadata?.metadata?.image;
